...
func (s *R2Store) Put(ctx context.Context, prefix Prefix, key string, size Size, data []byte) error {
	...
	_, err := s.client.PutObject(ctx, &s3.PutObjectInput{
		...
		Key:          aws.String(objectPath(prefix, key, size)),
...
// Delete verilen boyutlardaki objeleri siler. R2'de dizin yok — her boyut
// ayrı objedir, tek tek silinir. Olmayan obje hata değil — S3 DeleteObject
// zaten idempotent.
func (s *R2Store) Delete(ctx context.Context, prefix Prefix, key string, sizes []Size) error {
	if err := safeKey(key); err != nil {
		return err
	}
...
func (s *R2Store) URL(prefix Prefix, key string, size Size) string {
	return s.publicURL + "/" + objectPath(prefix, key, size)
}
