...
func (s *LocalStore) Put(ctx context.Context, prefix Prefix, key string, size Size, data []byte) error {
	...
	full := filepath.Join(s.baseDir, objectPath(prefix, key, size))
...
// Delete key'in dizinini komple siler; sizes'a bakmaz — dizinde zaten
// yalnızca o key'in boyutları var. Olmayan key hata değil — idempotent.
func (s *LocalStore) Delete(ctx context.Context, prefix Prefix, key string, _ []Size) error {
	if err := safeKey(key); err != nil {
		return err
	}
	dir := filepath.Join(s.baseDir, string(prefix), key)
...
func (s *LocalStore) URL(prefix Prefix, key string, size Size) string {
	return s.baseURL + "/" + objectPath(prefix, key, size)
}
